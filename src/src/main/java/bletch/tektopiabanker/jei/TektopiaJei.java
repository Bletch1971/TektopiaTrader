package bletch.tektopiabanker.jei;
...
public class TektopiaJei implements IModPlugin {
    ...
    public void register(IModRegistry registry) {
    	...
    	if (ModConfig.jei.showJeiInformationTab) {
    		try {
    			registerIngredientInfo(registry);
    		}
    		catch (Exception ex) {
	...
	private static void registerIngredientInfo(IModRegistry registry) {
		ArrayList<String> processed = new ArrayList<String>();
		
		ModDetails.MOD_LOGGER.info("Registering item/block information with JEI");
		
		ArrayList<ItemStack> modItemStacks = new ArrayList<ItemStack>();
		modItemStacks.addAll(ModUtils.getModBlockStacks());
		modItemStacks.addAll(ModUtils.getModItemStacks());
		
		ArrayList<String> missingInformation = new ArrayList<String>();
		ArrayList<String> registeredInformation = new ArrayList<String>();		
		int count = 0;
		
    	for (ItemStack modItemStack : modItemStacks) {
        
			NonNullList<ItemStack> itemStackList = NonNullList.create();

			if (modItemStack.getHasSubtypes()) {
				Item item = modItemStack.getItem();
				item.getSubItems(item.getCreativeTab(), itemStackList);
    		} else {
    			itemStackList.add(modItemStack);
    		}
			...
			for (ItemStack itemStack : itemStackList) {
	        	...
	        	// check if the item belongs to the Mods domain

...
public class BlockTop {
	...
	public static class getTheOneProbe implements com.google.common.base.Function<ITheOneProbe, Void> {
		...
		public Void apply(ITheOneProbe theOneProbe) {
			...
			probe.registerProvider(new IProbeInfoProvider() {
				...
				public void addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, EntityPlayer player, World world, IBlockState state, IProbeHitData data) {
					...
					if (block.getRegistryName().getResourceDomain().equalsIgnoreCase(ModDetails.MOD_ID)) {
						BlockTop.getTheOneProbe.addProbeInfo(mode, probeInfo, player, world, state, data);
					}	
				}
			...
			probe.registerBlockDisplayOverride((mode, probeInfo, player, world, state, data) -> {
				...
				if (block.getRegistryName().getResourceDomain().equalsIgnoreCase(ModDetails.MOD_ID)) {
					return BlockTop.getTheOneProbe.overrideStandardInfo(mode, probeInfo, player, world, state, data);
				}
		...
		private static boolean addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, EntityPlayer player, World world, IBlockState state, IProbeHitData data) {
			...
			if (!itemStack.getItem().getRegistryName().getResourceDomain().equalsIgnoreCase(ModDetails.MOD_ID)) {
				if (tileEntity != null) {
